<template>
  ...
  <table style="width: 100%;margin-top: 100px;max-height: 300px;display: block;overflow:scroll;min-width: 1500px">
    <tr>
      <td>index</td>
  ...
  <table style="width: 100%;margin-top: 100px;max-height: 300px;display: block;overflow:scroll;min-width: 1500px">
    ...
    <tr v-for="(row, $index) of tableData" :key="row.id">
      ...
      <td width="120">
        <lazy-component :id="row.id" :key="row.id" lazyKey="table4">
          <div v-if="!row.hidden" :style="{color: $index % 2 === 0 ? 'red' : 'blue'}">Operations{{ row.id }}</div>
          <template #loading>
            <div style="height: 40px">loading{{ row.id }}</div>
          </template>
